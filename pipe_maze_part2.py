...
class Maze:
    ...
    def find_start_directions(self):
        ...
        for di, dj in [WEST, EAST, SOUTH, NORTH]:
            ni, nj = self.start_position[0] + di, self.start_position[1] + dj
            if (
                    0 <= ni < len(self.maze) and 0 <= nj < len(self.maze[0])
                    and ANTI_DIRECTION[di, dj] in DIRECTION[self.maze[ni][nj]]
            ):
    ...
    def create_loop(self) -> int:
        ...
        :return: number of  steps from the starting position to the point farthest from the starting position
        """
        queue: list[tuple[tuple, tuple, int]] = [*self.start_directions]  # (coordinates, exit direction, distances)
        ...
        while True:
            ...
            self.track_maze[i][j] = self.maze[i][j]

            if (ni, nj) in self.visited:
                self.track_maze[ni][nj] = self.maze[ni][nj]
                return distance + 1
    ...
    def calculate_enclosed_area(self) -> int:
        ...
        If it is not part of a big loop, count the number of pipe crossings.
